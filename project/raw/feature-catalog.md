...
| Member functions | Current receiver-call behavior is owned by the function-invocation owner and receiver qualification by [Zax qualifiers](../../language/qualifiers.md); complete `once`, capture, reassignment, and the relationship between `once` and `operator type` remain future work |
...
| Explicitly sized integers, floating points, enums, booleans, and strings | Current finite integer types, representations, role identities, conversions, and operation domains are owned by [Zax integers](../../language/integers.md); enum identity/validity remains in [enum types](enum-types.md), while floating, fixed-point, unbounded, and deferred numeric mechanics remain in [numeric type families](numeric-type-families.md) |
